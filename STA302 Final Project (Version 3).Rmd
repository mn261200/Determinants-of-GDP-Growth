...
df <- gov_exp %>% 
  ...
  na.omit() %>% 
  mutate(is_western = ifelse(Country.Name %in% western$Country.Name, 1, 0)) %>% 
  set_colnames(c("country_name","gov_exp","life_avg","gdp_grow","sec_edu","pol_stal","rol","gov_eff", "is_western"))
...
We will be using 7 predictors and 1 response in total for this dataset, with 3 predictors  from the World Bank Development Indicators (WDI), and 3 from the World Bank Governance Indicators (WGI). The final predictor is a binary variable that will indicate whether a country is Western or not.
...
While our response seems normally distributed, Figure 2 suggests to us that a lot of our predictors may probably be skewed. For instance, Political Stability, Rule of Law, and Government Effectiveness seem to all be right-skewed, while Life Expectancy and Government Expenses are left-skewed. This may present itself as a violation of Linearity and Homoscedasticity. 

But this also suggests to us that some predictors may be repairable, as some of the skews seem to be influenced by outlier points. For instance, Government Expenses, Government Effectiveness, and Political Stability all seem to have points that stray far from the main cluster. We will check whether these outlier points are influential, so that we can decide whether to remove them later. 

## First Model

We first fit a model that utilizes all 7 of our predictors. Additionally, we interact 6 numerical predictors in the model with 1 binary predictor as mentioned in our Methods section. This gives us a total of 13 predictors. Before interpreting the results of our model, we must verify it's assumptions.
...
Our Residuals (see Appendix 3) show us that while most predictors seem to have random scatter, the problems discussed in Figure 2 arise again. Namely, the residuals of Government Expenses and Political Stability seem to be heavily influenced by a handful of observations. This presents itself as a possible violation of Linearity and Homoscedasticity. But rather than doing Box Cox Transformations, Figure 2 suggests to us that we should at least attempt to check if they are influential observations before doing anything else.

## Identifying Influential Observations

Will utilize the tools mentioned in our Methods section to see whether the aforementioned outlier points are influential. Below are the results of running, DFFITS, and DFBETAS on our model. 

We can see that observation 24 and 32 appear in at least 5 of the 8 plots. Using a soft rule of greater than half, this is convincing enough for us to remove them. 
...
model1 <- lm(train_gdp_grow ~ train_gov_exp*train_is_western + train_life_avg*train_is_western + train_sec_edu*train_is_western + V5*train_is_western + V6*train_is_western + V7*train_is_western, data=train_df)
summary(model1)
```
...
An important sidenote is that with the inclusion of interaction terms, multicollinearity increases by an extraordinary amount for each of our 13 predictors. However, this is to be expected, as 
...
# plot(cooksd, type = "h", cex=2, main="Influential Obs. by Cook's ", ylab = "Cook's Distance", xlab = "Column Number (in training dataset)")  # plot cook's distance
# abline(h = qf(0.5, p+1, n-p-1), lty = 2, col="red")  # add cutoff line
# text(x=1:length(cooksd)+1, y=cooksd, labels=ifelse(cooksd>qf(0.5, p+1, n-p-1), names(cooksd),""), col="red")  
...
train_df_v2 <- train_df[-c(4,24,32),]
...
```{r, echo=FALSE}
apss <- regsubsets(train_gdp_grow ~ train_gov_exp*train_is_western + train_life_avg*train_is_western + train_sec_edu*train_is_western + V5*train_is_western + V6*train_is_western + V7*train_is_western, data=train_df_v2, nvmax = 13)
summary(apss)
```
...
Notice that since the max Adjusted $R^2$ score is at 8 predictors, we start to get penalized for adding more than 8 predictors, each of which don't explain an adequate amount of variation.  Thus, we will attempt to keep our model at 8 predictors or less. Since the model with maximum BIC has 5 predictors, our these will be our two candidate models that we will compare. 

```{r, echo=FALSE}
# model_r2 <- lm(train_gdp_grow ~ train_gov_exp + train_life_avg + train_is_western + V6 + V7 + train_is_western:V5 + train_is_western:V6 + train_is_western:V7, data = train_df_v2)
# 
# model_bic <- lm(train_gdp_grow ~ train_gov_exp:train_is_western + train_is_western:train_sec_edu + train_is_western:V5 + train_is_western:V7 + train_life_avg, data = train_df_v2)

model3 <- lm(train_gdp_grow ~ train_gov_exp + V5 + V7 + train_is_western:train_sec_edu + train_is_western:V6 + train_life_avg, data = train_df_v2)
